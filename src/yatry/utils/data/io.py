...
def create_random_passengers(
    ...
    for i in range(n_passengers):
        ...
        while  not(Location.IISERB in (origin, destination) and origin is not destination):
...
pprint(create_random_passengers(n_passengers=5, time_range=(datetime.now(), datetime.now()+timedelta(days=1))))
