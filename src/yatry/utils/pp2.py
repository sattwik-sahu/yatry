...
def main():
    ...
    saving = []
    x_vals = []
    for N_PASSENGERS in range(99,100):
        ...
        sns.heatmap(affinity_matrix, cmap="viridis", cbar_kws={'label': 'Affinity Score'})
        ...
        for auto_number, (val, idxs) in enumerate(sorted(groups.items()), start=1):
            ...
            try:
                ...
                dep_time = float(np.mean([p.get_dep_time_range_num()[0] for p in group]))
